Cypress.Commands.add('login', (username = 'USERNAME', password = 'PASSWORD') => {
    ...
    cy.get('div.width-lg-auto > :nth-child(2) > .d-inline-block', { timeout:  50000 }).click().log('Botão para acesso a pagina de Login clicado!')
    cy.get('#login_field', { timeout:  50000 }).should('be.visible').type(Cypress.env(username)).log('Usuário incluído no campo de username!')
    cy.get('#password', { timeout:  50000 }).should('be.visible').type(Cypress.env(password)).log('Senha Incluída no campo de password!')
