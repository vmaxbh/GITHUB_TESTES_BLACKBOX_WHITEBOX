Cypress.Commands.add('login', (username = 'USERNAME', password = 'PASSWORD') => {
    ...
    cy.get('#login_field', { timeout:  10000 }).should('be.visible').type(Cypress.env(username)).log('Usuário incluído no campo de username!')
    cy.get('#password', { timeout:  10000 }).should('be.visible').type(Cypress.env(password)).log('Senha Incluída no campo de password!')
